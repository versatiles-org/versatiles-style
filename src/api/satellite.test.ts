...
describe('satellite()', () => {
	...
	it('applies custom base URL to satellite tiles', () => {
		const style = satellite({ urls: { base: 'https://my.cdn.com' } });
		const src = style.sources['satellite'] as { tiles: string[] };
		expect(src.tiles[0]).toContain('my.cdn.com');
	});

	it('accepts explicit satellite URL string', () => {
		const style = satellite({ urls: { satellite: 'https://sat.tiles/{z}/{x}/{y}' } });
		const src = style.sources['satellite'] as { tiles: string[] };
		expect(src.tiles[0]).toBe('https://sat.tiles/{z}/{x}/{y}');
	});

	it('accepts satellite TileJSONSpecification', () => {
		const tileJSON = {
			tiles: ['https://sat/{z}/{x}/{y}'],
			minzoom: 0,
			maxzoom: 18,
		} as TileJSONSpecification;
		const style = satellite({ urls: { satellite: tileJSON } });
