...
const $land          = "#0b0a0a";
const $water         = "#18181b";
const $boundary      = "#271f2e";
const $label         = "#3d293d";
const $labelhalo     = "#000000";
const $street        = "#000000";
const $streetoutline = "#1a1a1a";
const $wood          = "#0c120d";
const $rail          = "#281b28";
const $agriculture   = "#120707";
const $grass         = "#171915";
const $site          = "#141619";
const $building      = "#1a0e1a";
const $foot          = "#100613";
...
export default {
	...
	"boundary-country:outline": {
		size: { 2: 2, 10: 6, },
		opacity: { 2: 0, 4: 0.3 },
		color: color($land).darken(0.05).hex(),
		lineBlur: 1,
	},
	"boundary-country": {
		size: { 2: 1, 10: 4, },
	...
	"land-{park,garden,vegetation}": {
		color: color($grass).darken(0.5).saturate(0.2).hex(),
		opacity: { 11: 0, 12: 1 },
	},
	"land-agriculture": {
		color: $agriculture,
	...
	"land-{commercial,industrial,residential}": {
		color: color($land).lighten(0.03).hex(),
		opacity: { 10: 0, 11: 1 },
	},
	...
	"bridge": {
		color: color($land).lighten(0.01).hex(),
	},
	...
	"tunnel-street-*": {
		color: color($street).lighten(0.03).hex(),
	},
	"tunnel-street-*:outline": {
		color: color($streetoutline).lighten(0.13).hex(),
		lineDasharray: [ 1, 2 ],
	},
	"bridge-street-*:outline": {
		color: color($streetoutline).lighten(0.15).hex(),
	},
	// motorway
	...
	"way-{footway,path,steps}:outline": {
		...
		opacity: { 17: 0, 18: 1 },
		minzoom: 17,
		color: color($foot).lighten(0.05).hex(),
	},
	...
	"{tunnel-,bridge-,}transport-{rail,lightrail}": {
		color: color($rail).darken(0.1).hex(),
		size: { 8: 1, 12: 1, 15: 2 },
		lineDasharray: [ 2, 2 ],
	},
	// bridge
	...
	"label-place-suburb": {
		...
		size: { 11: 11, 13: 14 },
		textTransform: "uppercase",
		color: color($label).rotate(-30).saturate(0.5).lighten(0.5).hex(),
	},
	"label-place-quarter": {
		minzoom: 13,
		size: { 13: 13 },
		textTransform: "uppercase",
		color: color($label).rotate(-40).saturate(0.5).lighten(0.5).hex(),
	},
	"label-place-neighbourhood": {
		minzoom: 14,
		size: { 14: 12 },
		textTransform: "uppercase",
		color: color($label).rotate(-50).saturate(0.5).lighten(0.5).hex(),
	},
