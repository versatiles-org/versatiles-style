...
// translate root properties to style properties
const props = {
	// paint
...
export default function decorate(layers, style) {
	...
	return Object.entries(structuredClone(layers)).map(function ([id, layer]) {
		...
		Object.values(rules).filter(
			rule => minimatch(matchid, rule.matchid) && pseudo === rule.pseudo
			...
			if (rule.shortcut) {
				switch (layer.type) {
					...
					case "symbol":
						...
						if (rule.shortcut.icon) rule.layout["icon-image"] = rule.shortcut.icon;
