...
import maplibre_style_properties from './maplibre_properties.js';
...
export default class Stylemaker {
	...
	#decorateLayers(options = {}) {
		...
		let layerStyleRules = this.#layerStyleGenerator({
			colors: getLookupProxy('color', colorLookup),
			fonts: getLookupProxy('font', fontLookup),
			fields: getLookupProxy('field', fieldLookup),
		})

		function getLookupProxy(name, lookup) {
			const handler = {
				get(target, key, receiver) {
					let value = lookup.get(key);
					if (!value) throw new Error(`unknown ${name} name: ${name}.${key}`)
					return value
				},
			};
