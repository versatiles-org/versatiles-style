...
export default class Stylemaker {
	...
	build(options) {
		...
		const style = deepMerge(this.#template, {
			id: this.#id,
			name: this.#id,
		});
	...
	#decorateLayers(options = {}) {
		...
		let layers = deepClone(this.#layers).flatMap(layer => {
			...
			processShortcuts(layer, layerStyle);
		...
		function processShortcuts(layer, layerStyle) {
			switch (layer.type) {
				case 'background': [
			...
			// delete shortcuts;
			delete layerStyle.color
			delete layerStyle.size
			delete layerStyle.text
			delete layerStyle.font
			delete layerStyle.icon
			delete layerStyle.opacity

			function resolveShortcut([keyShortcut, keyCorrect]) {
				if (layerStyle[keyShortcut] !== undefined) layerStyle[keyCorrect] = layerStyle[keyShortcut];
			}
		}
